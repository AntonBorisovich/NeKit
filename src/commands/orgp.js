...
// == Интерактивное формирование аргументов
...
class Orgp {
    ...
    async run(nek, client, msg, args){
		...
		let orgpArgs = {};
		if (!args[0]) { // если не был дан никакой аргумент, то начать интерактивное заполение аргументов
		...
		} else { // иначе, если есть текстовые аргументы, то обработать их
			orgpArgs.transType = false;
